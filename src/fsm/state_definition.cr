module FSM
  # A state in the machine definition. Renamed from the
  # old `State` now that `State` names the runtime snapshot.
  #
  ...
  # object across every interpreter and fiber. Once sealed, its
  # builder methods raise SealedStateError, which is what keeps a shared definition
  # immutable after build.
  class StateDefinition(T)
    getter id : String

    # Multiple transitions per event, distinguished by guards. Each on_event for the
    # same event appends, and resolution walks the array
    # in registration order. A guardless transition
    ...
    # Per-event blocked handlers, keyed by event. A handler fires when the event has
    # candidate transitions but every guard
    # rejected. The keying is per event because different blocked events want
    # different messages; on_unknown_event is state-wide by contrast. The handler
    # receives the event, the context, and the
    ...
    # The state-wide unknown-event handler. A
    ...
    # events individually is a contradiction. This is the
    ...
    # Whether the owning machine has sealed this definition.
    @sealed : Bool = false
    ...
    # from outside the FSM module. The Builder constructs
    ...
    # guard rejected. It is keyed per event
    ...
    # without re-evaluating any guard. Registering a second
    ...
    # registered on this state at all. It is
    ...
    # Register a transition from this state to a target on an event. Calling on_event
    # repeatedly for one event appends,
    # building the ordered candidate list resolution walks.
    ...
    # validate that every target names an existing state.
    def all_target_states : Array(String)
      @transitions.values.flatten.map(&.target).uniq
    end

    # Resolve the outcome for an event in this context. An unregistered event is
    # EventNotRecognized; otherwise the
    ...
    protected def resolve(event : String, context : T) : TransitionFound(T) | TransitionsBlocked | EventNotRecognized
      ...
      # second guard pass. This array is freshly allocated on
    ...
    # Seal this definition and every transition it owns.
    ...
    # plan's action list is positional regardless of registration.
    protected def exit_callback : (String, T) ->
      @exit_callback || ->(_event : String, _context : T) { }
    end

    # The entry callback as a real proc, a no-op when none is registered.
    ...
    # `plan` uses its presence to decide whether a
    ...
    # The state-wide unknown-event handler, or nil when none is registered.
    # `plan` uses its presence to decide whether an
