module FSM
  # The shared observer-firing step for both interpreters (design section 9, section
  # 10.3 step 20, D19). Service(T) and AsyncService(T) include this so the firing
  ...
  module ObserverFiring
    ...
    # nil (design section 9, section 10.3 step 20, D19). Each observer fires in its own
    # begin/rescue so an on_transition that raises never skips on_event_processed:
    # section 9 guarantees on_event_processed fires after every step. The FIRST
    ...
    # a failed step the snapshot already carries the first exception (design section 9),
    ...
    # stays authoritative and the returning path stays a value, not a raise (design
    # section 11). That silence is deliberate (design section 9). On a non-failed step
    ...
    private def fire_observers(snapshot : State, transitioned : Bool) : Exception?
      if snapshot.status.failed?
        ...
        # (design section 9, section 10.3 step 20).
        if transitioned
