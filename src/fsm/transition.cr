module FSM
  ...
  class Transition(T)
    ...
    # Whether this transition is internal. An internal
    ...
    # during build like a guard or callback, and frozen at seal.
    @internal : Bool = false

    # Whether the owning machine has sealed this transition.
    @sealed : Bool = false
    ...
    # the plan's action list is positional regardless of registration.
    protected def callback : (String, T) ->
      @callback || ->(_event : String, _context : T) { }
    end

    # Seal this transition. Idempotent.
