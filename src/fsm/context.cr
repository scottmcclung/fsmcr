module FSM
  # The hash-and-mutex bucket the library ships as one selectable T. It is not the
  # context mechanism; it is one context a caller
  ...
  # caller per-operation atomicity on a shared context. A
  ...
  class Context
    ...
    # which evaluates to its right-hand side. Holds the mutex for the write.
    ...
    # value stored. The read, the block, and the write all run
    # under one lock hold, which is what makes a compound read-modify-write atomic
    # against a shared context. Reads @data[key] before
    ...
    # deadlock against a concurrent step whose callback wants this same lock.
