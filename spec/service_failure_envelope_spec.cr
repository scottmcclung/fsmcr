...
# Service#send's failure envelope and observer independence (design section 9,
# section 10.3 step 20, section 11; D19; fsmcr-1uo).
#
...
#
#   1. The failure envelope covers planning. Guards run inside plan (design section
#      10.2 step 12), and planning is part of the step, so a guard that raises must
#      surface as a Failed snapshot on the returning path rather than propagating raw
#      (design section 11: "where a value comes back, failure is in the value"). The
...
#      then on_event_processed (design section 10.3 step 20). on_event_processed is
#      guaranteed to fire after EVERY step (design section 9), so an on_transition that
...
#      AsyncService, design section 8.2). The design-consistent surfacing, keeping the
...
# discarded and the first recorded exception wins (design section 9). That case is
...
describe "Service#send failure envelope and observer independence" do
  describe "a guard that raises (design section 10.2 step 12, section 11)" do
    it "returns a Failed snapshot carrying the guard's exception instead of propagating raw" do
      ...
      machine : FSM::Machine(TurnContext) = FSM::Machine(TurnContext).build("world") do |m|
        m.state("cave") do |s|
          # The guard runs inside plan (design section 10.2 step 12). Planning is part
          # of the step, so a raising guard must be caught by send's failure envelope
          # and surfaced as a Failed value (design section 11), not propagated raw.
    ...
    it "fires on_event_processed with the Failed snapshot and not on_transition (D19)" do
      ...
      # on_transition, because no transition completed (D19).
  ...
  describe "an on_transition observer that raises on a successful step (design section 9)" do
    it "still fires on_event_processed, keeps the transition committed, and re-raises the first exception" do
      ...
      # The first observer exception reaches the caller (design section 11: this is the
  ...
  describe "an on_event_processed observer that raises on a successful step (design section 9, section 11)" do
    ...
    # the design leaves propagating (design section 9, section 11).
