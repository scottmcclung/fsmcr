...
# The on_event_processed observer.
#
...
# A consumer writes state back to an entity after any event with
# on_event_processed and relocates an entity only on a real change with on_transition.
#
# The handler receives the State snapshot the step produced:
# the new state on success, the unchanged state on a blocked or unknown event,
# and the Failed snapshot on a step where a callback raised. The snapshot does
...
#
# On a failed step (a callback raised) on_transition does NOT fire,
# because no transition completed, while on_event_processed DOES fire and receives the
# Failed snapshot. An observer that itself raises while handling a failed step is
# an edge case the design defines by the invariant that the first exception is the one
# already recorded in the snapshot: the observer's own exception
...
# live service has no observer setter.
describe "on_event_processed" do
  it "fires after a successful transition, receiving the new-state snapshot" do
    ...
    if snapshot
      # The handler receives the snapshot the step produced: the new state.
  ...
  it "fires after every send, across blocked, unknown, and successful outcomes" do
    ...
    # One fire per send, regardless of outcome.
  ...
  it "fires on_event_processed with the Failed snapshot on a failed step, and not on_transition" do
    ...
    # The state pointer did not move.
  ...
  it "keeps the first recorded exception when the observer itself raises on a failed step" do
    ...
    service : FSM::Service(TurnContext) = FSM::Service(TurnContext).interpret(machine, "cave", ctx) do |observers|
      observers.on_transition { |_snapshot| transition_fired = true }
      # The observer captures the snapshot it was handed, then raises. The first
      # exception (the callback's) is the one already recorded in
  ...
  it "propagates the observer's own exception out of send on a non-failed (blocked) step" do
    ...
    service : FSM::Service(TurnContext) = FSM::Service(TurnContext).interpret(machine, "cave", ctx) do |observers|
      ...
      # so its snapshot status is Success. fire_event_processed runs the observer
      ...
      # which discards the observer's exception so the first recorded one wins,
      # is intentional and matches on_transition's pre-existing behavior.
  ...
  it "turns a raising on_blocked handler into a Failed step: on_event_processed sees it, on_transition does not" do
    ...
    machine : FSM::Machine(TurnContext) = FSM::Machine(TurnContext).build("world") do |m|
      m.state("cave") do |s|
        ...
        # handler fails the step part-way like any other callback. The step never
        # commits, so the pointer stays at "cave".
    ...
    # raising handler surfaces as a Failed value rather than a raise.
    result : FSM::State = service.send("north")
    ...
    # on_event_processed fired and received that same Failed snapshot.
    snapshot : FSM::State? = captured
    snapshot.should be_a(FSM::State)
    if snapshot
      snapshot.status.should eq FSM::Status::Failed
