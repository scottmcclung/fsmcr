...
# Test-only bridge to the pure core (design section 7, D7).
#
# `plan` is protected and Plan, Action, ActionKind, and the three outcome structs
# are not part of the public API (D7). The acceptance criteria still require a spec
...
# weakening any visibility in src (D7 says the surface must not be widened for the
# specs' convenience).
module FSM
  module PlanProbe
